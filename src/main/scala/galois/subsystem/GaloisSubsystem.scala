...
class GaloisSubsystemModuleImp[+L <: GaloisSubsystem](_outer: L) extends BaseSubsystemModuleImp(_outer)
  ...
  if (tvencoder.isDefined && traceConverter.isDefined) {
    ...
    BoringUtils.bore(outer.tiles(0).module.core.io.dmem.s1_data.data, Seq(traceConverter.get.dmem_data))
...
//    BoringUtils.bore(outer.tiles(0).module.core.rocketImpl.mem_npc, Seq(traceConverter.get.mem_npc))
    BoringUtils.bore(outer.tiles(0).module.core.io.imem.req.bits.pc, Seq(traceConverter.get.mem_npc))
    ...
    // Get output from MulDiv module
    BoringUtils.bore(outer.tiles(0).module.core.rocketImpl.div.io.resp.valid, Seq(traceConverter.get.mulDiv_valid))
